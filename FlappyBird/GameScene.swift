...
//        壁用のノード
        ...
        // アイテム用のノード ★
               ...
               // 効果音ファイルの読み込み ★
               soundAction = SKAction.playSoundFileNamed("coin03.mp3", waitForCompletion: true)
...
//                    衝突時に動かないように設定
    ...
    // アイテムを表示する ★
       func setupItem() {
        ...
        _ = SKSpriteNode(texture: itemTexture)

           // 壁の大きさを得るために画像を読み込む
           let wallTexture = SKTexture(imageNamed: "wall")
        _ = wallTexture.size().width
           let groundTexture = SKTexture(imageNamed: "ground")
        _ = groundTexture.size().height
           ...
           let createItemAnimation = SKAction.run({
            ...
            itemSprite.size = CGSize(width: itemSprite.size.width*0.2, height: itemSprite.size.height*0.2)
